...
#' @param Grps
#' @param CompCol_all
#' @param Min.PointsperPeak
#' @param peak.spotting.factor
#' @param Integration_baseL_factor
#' @param plan
#' @param Min.cor.w.main_adduct
#' @param Min.cor.w.M0
#' @param Min.iso.count
#' @param return_unsuc_searches
#' @param Min.Res
#'
#' @import foreach
#' @return
                            ...
                            return_unsuc_searches = FALSE)
{
  ...
  Output <-
    foreach(file = 1:length(files),
            .packages = c("lazypeaks", "data.table")) %dopar% {
                ...
                for (iso.run in c("MAiso", "LAisos")) {
                  ...
                  Bureau <-
                    lapply(seq(length(.ChromData)), function(i,
                      ...
                      ##################################
                      EIC.dt <- lazypeaks:::get_EIC_table(rt = unname(ChromData[[i]]@rtime),
