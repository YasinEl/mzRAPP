...
callmzRAPP <- function() {
  ...
  ui <- shinydashboard::dashboardPage(
    ...
    shinydashboard::dashboardBody(
      useShinyjs(),
      extendShinyjs(text = 'shinyjs.scrolltop = function() {window.scrollTo(0, 0)};'),
      #always start from top of panel
      ...
      shinydashboard::tabItems(
        shinydashboard::tabItem(
          tabName = "Readme",
          shiny::tags$div(class = "rmd-class",
                          shiny::includeHTML(
                            system.file(
                              "md",
                              "README.html",
        ...
        shinydashboard::tabItem(
          ...
          shiny::fluidRow(shiny::column(
            ...
            shiny::p(
              "(please note that dialog boxes to select files might not be directly visible as they tend to open behind the R console. Try to minimize your windows if it dialog boxes do not appear after clicking.)"
            ),
            shiny::a(
              "Click here for information on how to prepare csv files and set paramters.",
          ...
          shiny::fluidRow(
            shiny::conditionalPanel(condition = "input.use_envipat_res_list",
                                    shiny::column(
                                      2,
                                      shiny::verbatimTextOutput(outputId = 'custom_res_mz', placeholder = TRUE)
                                    ))
          ),
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::p(
                "mz precision corresponds to the highest tolerable difference of mass peaks along the mz dimension in ppm. Typical values for orbitraps range from 5 to 8. For some TOF instruments this can go up to 20 ppm."
              ),
              shiny::p(
                "mz accuracy corresponds to the maximum allowed difference between the theoretical mz of an ion species and the intensity weighthed average of all mz values over a chromatographic peak in ppm. Those values are typicaly between 3 and 5 for orbitrap as well as TOF instruments."
              ),
              shiny::br()
            )
          ),
          ...
          shiny::fluidRow(
            shiny::column(
              2,
              shiny::numericInput('percision_mz_tol_input', 'mz precision [ppm]', 5, step = 0.1)
            ),
            shiny::column(
              2,
              shiny::numericInput('accurate_MZ_tol_input', 'mz accuracy [ppm]', 5, step = 0.1)
            )
          ),
          ...
          shiny::fluidRow(shiny::column(
            ...
            shiny::br(),
            shiny::p(
              "(depending on the number of files and compounds this can take some time (minutes to hours))"
            )
          )),

          shiny::fluidRow(shiny::column(
            2,
            shiny::actionButton('generate_benchmark', 'Generate benchmark', style = "background-color: #d2f8fa")
          )),
        ),
        ...
        shinydashboard::tabItem(
          ...
          shiny::h4(
            paste(
              "In the following key data and overview graphics for the generated benchmark are provided. Please note that the benchmark dataset contains only peaks which were confirmable",
          ...
          shiny::a(
            "Click here for more information on how to proceed from here.",
            onclick = "openTab('Readme')",
            href = "#vBMID"
          ),
          ...
          shiny::fluidRow(shiny::column(
            10,
            offset = 1,
            shiny::tags$hr(style = "border-color: darkgray;")
          )),
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::h4(
                paste(
                  "In the following interactive scatter plot and histogram different benchmark peak variables can be explored. In order to have a look at the actual peaks you can ",
              ...
              shiny::a(
                "Click here for more information on how to proceed if you are not satisfied with the current benchmark dataset.",
                onclick = "openTab('Readme')",
                href = "#vBMID"
              )
            ),
            shiny::column(
              ...
              shiny::checkboxInput(
                inputId = 'Sky_add',
                label = 'Export only main adduct',
                value = TRUE,
                width = NULL
              ),
              shiny::checkboxInput(
                inputId = 'Sky_iso',
                label = 'Export only most abundant isotopologue',
                value = TRUE,
                width = NULL
              ),
              shiny::actionButton(
                inputId = 'Skyline_export',
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('graph_area_bench_overview') %>% shinycssloaders::withSpinner(color =
                                                                                                       "#0dc5c1", type = 4)
                )
              ),
              ...
              shiny::fluidRow(
                shiny::tags$style(
                  shiny::HTML(
                    "#bench_overview_input_x+ div>.selectize-dropdown {bottom: 100% !important;top:auto!important;}"
                  )
                ),
                shiny::tags$style(
                  shiny::HTML(
                    "#bench_overview_input_y+ div>.selectize-dropdown {bottom: 100% !important;top:auto!important;}"
                  )
                ),
                shiny::tags$style(
                  shiny::HTML(
                    "#bench_overview_input_color+ div>.selectize-dropdown {bottom: 100% !important;top:auto!important;}"
                  )
                ),
                shiny::column(
                  3,
                  shiny::selectInput(
                    'bench_overview_input_x',
                    'x-axis',
                    choice_vector_bench,
            ...
            shiny::column(
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('graph_area_bench_histo') %>% shinycssloaders::withSpinner(color =
                                                                                                    "#0dc5c1", type = 4)
                )
              ),
              ...
              shiny::fluidRow(
                shiny::tags$style(
                  shiny::HTML(
                    "#select_bench_histo+ div>.selectize-dropdown {bottom: 100% !important;top:auto!important;}"
                  )
                ),
                shiny::column(
                  8,
        ...
        shinydashboard::tabItem(
          ...
          shiny::fluidRow(shiny::column(
            12,
            shiny::strong('1. Select used tool', style = "font-size:30px"),
            shiny::br(),
            shiny::br()
          )),
          ...
          shiny::fluidRow(
            shiny::column(
              12,
              shiny::strong('2. Select unaligned and aligned files', style = "font-size:30px"),
              shiny::br(),
              shiny::a(
                "Click here for information on how to get unaligned and aligned files from different NPP tools.",
                onclick = "openTab('Readme')",
                href = "#sNPP_readme"
              ),
              shiny::br()
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::div(
                style = 'width:190px',
                shiny::verbatimTextOutput(outputId = 'ug_upload_files', placeholder = TRUE)
              ),
              shiny::div(style = "width: 20px;"),
              shiny::div(
                style = 'width:190px',
                shiny::verbatimTextOutput(outputId = 'g_upload_file', placeholder = TRUE)
              ),
              shiny::div(style = "width: 20px;")
            )
          ),
          ...
          shiny::fluidRow(shiny::column(
            12,
            shiny::strong('3. Select benchmark and/or options file', style = "font-size:30px"),
            shiny::br(),
            shiny::br()
          )),
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              style = "display: inline-flex;",
              shinyWidgets::prettySwitch(
                inputId = 'use_generated_benchmark',
            ...
            shiny::column(
              ...
              shiny::div(
                ...
                shiny::conditionalPanel(
                  condition = "!input.use_generated_benchmark",
                  shiny::actionButton(
                    inputId = 'benchmark_upload',
              ...
              shiny::div(
                style = "width:190px",
                shiny::conditionalPanel(
                  condition = "!input.use_generated_options",
                  shiny::actionButton(
                    inputId = 'options_upload',
            ...
            shiny::column(
              ...
              shiny::div(
                ...
                shiny::conditionalPanel(
                  ...
                  shiny::div(
                    style = 'width:190px',
                    shiny::verbatimTextOutput(outputId = 'benchmark_upload_file', placeholder = TRUE)
                  ),
                )
              ),
              shiny::div(style = "width: 20px;"),
              shiny::div(
                ...
                shiny::conditionalPanel(
                  ...
                  shiny::div(
                    style = 'width:190px',
                    shiny::verbatimTextOutput(outputId = 'options_upload_file', placeholder = TRUE)
                  )
                )
              )
            )
          ...
          shiny::fluidRow(shiny::column(
            12,
            shiny::strong('4. Start NPP assessment', style = "font-size:30px"),
            shiny::br(),
            shiny::br()
          )),
        ...
        shinydashboard::tabItem(
          ...
          shiny::h4(
            paste(
              "Key performance measures are given for different stages of the NPP workflow. Empirical confindence intervals (alpha = 0.95) of calculated percentages are given in brackets",
              "(estimated via bootstrapping of molecules with R = 1000). For details on how individual performance measures are calculated please check the original mzRAPP puplication or readme."
            )
          ),
          shiny::a(
          ...
          shiny::fluidRow(shiny::downloadButton("report", "Generate report"), align =
                            "center"),
          #       shiny::fluidRow(
          #         # A static infoBox
          ...
          #       ),
          shiny::h4(
            paste("For more details please check the interactive plots below.")
          ),
          ...
          shiny::column(
            10,
            offset = 1,
            shiny::h2("Distribution of found/not found peaks")
          ),
          ...
          shiny::column(
            ...
            offset = 1,
            shiny::h4(
              "In the following interactive scatter plot and histogram the distribution of",
              shiny::tags$span(style = "color:blue", "found") ,
              "/",
              shiny::tags$span(style = "color:red", "not found"),
              "peaks can be investigated as a function of different benchmark peak variables.",
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('overview_plot') %>% shinycssloaders::withSpinner(color =
                                                                                           "#0dc5c1", type = 4)
                )
              ),
            ...
            shiny::column(
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('graph_area_3') %>% shinycssloaders::withSpinner(color =
                                                                                          "#0dc5c1", type = 4)
                )
              ),
          ...
          shiny::column(
            10,
            offset = 1,
            shiny::h2("Quality of reported NPP peak abundances")
          ),
          shiny::column(
            8,
            offset = 2,
            shiny::h4(
              paste0(
                "The quality of reported peak abundances is important in order to determine molecular compositions via isotopologue ratios or compare abundances between ",
          ...
          shiny::column(
            ...
            shiny::fluidRow(
              shiny::column(
                12,
                plotly::plotlyOutput('graph_area_2') %>% shinycssloaders::withSpinner(color =
                                                                                        "#0dc5c1", type = 4)
              )
            )
          ),
          ...
          shiny::column(
            ...
            shiny::h4(
              paste0(
                "The nature of missing values is of outmost importance for the choice of a fitting missing value imputation method. Since features defined in the benchmark could ",
          ...
          shiny::fluidRow(
            shiny::column(
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('graph_area_1') %>% shinycssloaders::withSpinner(color =
                                                                                          "#0dc5c1", type = 4)
                )
              )
            ),
          ...
          shiny::column(
            ...
            shiny::h4(
              paste0(
                "The alignment process is responsible for assembling peaks of different samples into features. mzRAPP is counting the minimum number ",
          ...
          shiny::fluidRow(
            shiny::column(
              3,
              offset = 1,
              shiny::tableOutput('error_count'),
              style = "overflow-y:scroll; height:464px"
            ),
            shiny::column(
              7,
              ...
              shiny::fluidRow(
                shiny::column(
                  12,
                  plotly::plotlyOutput('graph_hm_split') %>% shinycssloaders::withSpinner(color =
                                                                                            "#0dc5c1", type = 4)
                )
              ),
              ...
              shiny::fluidRow(
                shiny::column(
                  4,
                  shinyWidgets::pickerInput('mol_a', 'Molecule', c(), options = list(`live-search` =
                                                                                       TRUE))
                ),
                shiny::column(
                  3,
                  shinyWidgets::pickerInput('add_a', 'Adduct', c(), options = list(`live-search` =
                                                                                     TRUE))
                )
              )
            )
  ...
  server <- function (input, output, session) {
    ...
    mzML_filter <-
      matrix(c('mzML Files (*.mzML)', '*.mzML'),
             nrow = 1,
             ncol = 2)
    csv_filter <-
      matrix(
        c('Files (*.csv, *.txt, *.Rda)', '*.csv;*.txt;*.Rda'),
        nrow = 1,
        ncol = 2
      )
    ...
    #Benchmark
    mzML_files <- reactive({
      if (input$mzML_upload == 0) {
        return(NULL)
      }
      else {
        files <-
          tcltk::tk_choose.files(caption = 'Select .mzML files',
                                 multi = TRUE,
                                 filters = mzML_filter)
        if (length(files) > 1) {
          output$mzML_upload_files <-
            renderText(paste0(length(files), ' Files selected'))
        } else {
        ...
        file <-
          paste(
            tcltk::tk_choose.files(
              caption = 'Select sample-group file',
              multi = FALSE,
              filters = csv_filter
            ),
            collapse = " "
          )
        output$grps_upload_file <-
          renderText(paste0(basename(file)))
        return(file)
      }
    })

    coi_file <- reactive({
      if (input$coi_upload == 0) {
        return(NULL)
      }
      else {
        file <-
    ...
    observeEvent(input$Skyline_export, {
      ...
      cat(
        paste0(
          ...
          "Mass Accuracy to ",
          round_woe(max(benchmark_data$peaks.mz_span_ppm) / 2, 1),
        ...
        file = "Skyline_Instructions_For_Import.txt",
        sep = "\n"
      )
      ...
      shinyWidgets::sendSweetAlert(
        ...
        text = paste0(
          'The Skyline Transition list and peak boundaries have been exported as csv files to your current working directoy (',
          getwd(),
    ...
    #Comparison
    ug_files <- reactive({
      if (input$ug_upload == 0) {
        return(NULL)
      }
      else {
        files <-
          tcltk::tk_choose.files(caption = 'Select unaligned file(s)',
                                 multi = TRUE,
                                 filters = csv_filter)
        if (length(files) > 1) {
          output$ug_upload_files <-
            renderText(paste0(length(files), ' Files selected'))
        } else {
    ...
    benchmark_file <- reactive({
      if (input$benchmark_upload == 0) {
        ...
        file <-
          paste(
            tcltk::tk_choose.files(
              caption = 'Select benchmark file',
              multi = FALSE,
              filters = csv_filter
            ),
            collapse = " "
          )
        output$benchmark_upload_file <-
          renderText(paste0(basename(file)))
        return(file)
      }
    })
    options_file <- reactive({
      if (input$options_upload == 0) {
        return(NULL)
      }
      else {
        file <-
          paste(
            tcltk::tk_choose.files(
              caption = 'Select options file',
              multi = FALSE,
              filters = csv_filter
            ),
            collapse = " "
          )
        output$options_upload_file <-
          renderText(paste0(basename(file)))
        return(file)
      }
    })
    ...
    observeEvent(input$generate_benchmark, {
      ...
      tryCatch({
        ...
        if (is.null(grps_file()) || length(grps_file()) == 0) {
          ...
          missing_cols <-
            setdiff(c("sample_name", "sample_group"), colnames(grps))
          if (length(missing_cols) > 0) {
            stop(paste0(
              "Sample_group table is lacking shiny::columns: ",
              paste(missing_cols, collapse = ", ")
            ))
          }

          if (nrow(grps[is.na(sample_name) |
                        is.na(sample_group)]) > 0) {
            stop(
              "Values in sample_group table must not be empty! If you have only one group please add the same sample_group value for each sample!"
            )
          }
        ...
        if (is.null(coi_file()) || length(coi_file()) == 0) {
          ...
          if (length(missing_cols) > 0) {
            stop(paste0(
              "Target.table is lacking shiny::columns: ",
              paste0(missing_cols, collapse = ", ")
            ))
          }


          if (!is.numeric(targets$StartTime.EIC) |
              !is.numeric(targets$EndTime.EIC)) {
            stop(
              "Some values in your shiny::columns StartTime.EIC/EndTime.EIC are not numeric!"
            )

            if (nrow(targets[StartTime.EIC < 0 |
                             EndTime.EIC < 0 | EndTime.EIC < StartTime.EIC]) > 0) {
              stop(
                "Values for StartTime.EIC and EndTime.EIC cannot be lower than 0. Also values for StartTime.EIC have to be lower than those for EndTime.EIC!"
              )
            }

          }

          if ("user.rtmin" %in% colnames(targets) |
              "user.rtmax" %in% colnames(targets)) {
            if (!all(c("user.rtmin", "user.rtmax") %in% colnames(targets))) {
              stop(
                "Your target table includes only one of the shiny::columns user.rtmin/user.rtmax. If one is there you also need the other!"
              )
            }

            if (!is.numeric(targets$user.rtmin) |
                !is.numeric(targets$user.rtmax)) {
              stop(
                "Some values in your shiny::columns user.rtmin/user.rtmax are not numeric!"
              )
            }

            if (nrow(targets[user.rtmin < StartTime.EIC |
                             user.rtmax > EndTime.EIC]) > 0) {
              stop(
                "Values for user.rtmin/user.rtmax cannot be lower/higher than values for StartTime.EIC/EndTime.EIC respectivly!"
              )
            }

          }

          if (!is.character(targets$molecule)) {
            targets$molecule <- as.character(targets$molecule)
          }
        }

        #Resolution
        if (input$use_envipat_res_list == FALSE) {
          if (input$resolution_drop == '------') {
            stop('Please select used Resolution')
          }
          res_input <- input$resolution_drop
          resolution_df <-
            as.data.frame(resolution_list[[res_input]])
        } else {
          if (is.null(res_file()) || length(res_file()) == 0) {
            stop('No Resolution/mz file selected')
          }
          resolution_df <- fread(res_file())
          if (!all(c("m/z", "R") %in% c(colnames(resolution_df)))) {
            stop('Columns in Res/mz file missing')
          }
          resolution_df <- na.omit(resolution_df[, c("m/z", "R")])
          if (nrow(resolution_df) < 10) {
            stop('Not enough values in Res/mz file. Please provide at least 10 (better more).')
          }
        }

        withProgress(message = 'Calculation in progress',
                     detail = "calculating isotopologues...",
                     value = 0,
                     {
                       starttime <- Sys.time()
                       ...
                       message(paste0(
                         "Benchmark dataset has been exported to ",
                         getwd(),
                         "/Peak_list.csv"
                       ))
        ...
        shinyWidgets::sendSweetAlert(
          ...
          text = paste0(
            'Benchmark geneneration has been finished in ',
            round(proc.time, 0),
            ' min. The output has been saved to your
                                  working directory as ',
            getwd(),
            '/Peak_list.csv. It can be inspected by importing it to Skyline with the instructions
      ...
      if (!is.null(benchmark_data)) {
        ...
        output$size_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Benchmark size"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "# of molecules: ",
                                      length(unique(benchmark_data$molecule)),
                                      shiny::br(),
                                      "# of features: ",
                                      nrow(unique(
                                        benchmark_data, by = c("molecule", "adduct", "isoab")
                                      )),
                                      shiny::br(),
                                      "# of peaks: ",
                                      nrow(benchmark_data)
        ...
        output$chrom_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Peak rt-metrics"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "Median FWHM [s]: ",
                                      round_woe(median(
                                        benchmark_data$peaks.FW50M, na.rm = TRUE
                                      ), 0),
                                      shiny::br(),
                                      "Median # of points: ",
                                      median(benchmark_data$peaks.PpP, na.rm = TRUE)
                                    )
                                  )),
            color = "blue",
            fill = TRUE
        ...
        output$mz_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Peak m/z-metrics"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "Median mz accuracy [ppm] | abs: ",
                                      round_woe(
                                        median(benchmark_data$peaks.mz_accuracy_ppm, na.rm = TRUE),
                                        1
                                      ),
                                      " | ",
                                      round_woe(
                                        median(benchmark_data$peaks.mz_accuracy_abs, na.rm = TRUE),
                                        4
                                      ),
                                      shiny::br(),
                                      "Median mz range [ppm] | abs: ",
                                      round_woe(
                                        median(benchmark_data$peaks.mz_span_ppm, na.rm = TRUE),
                                        1
                                      ),
                                      " | ",
                                      round_woe(
                                        median(benchmark_data$peaks.mz_span_abs, na.rm = TRUE),
                                        4
                                      )
                                    )
                                  )),
    ...
    observeEvent({
      benchmark_data()
      input$bench_overview_input_x
      input$bench_overview_input_y
      input$bench_overview_input_color
    }, {
      benchmark_data <- isolate(benchmark_data())
      if (!is.null(benchmark_data)) {
        output$graph_area_bench_overview <-
          plotly::renderPlotly(
            plot_bench_overview(
              benchmark_data,
              input$bench_overview_input_x,
    ...
    observeEvent({
      benchmark_data()
      input$select_bench_histo
    }, {
      benchmark_data <- isolate(benchmark_data())
      if (!is.null(benchmark_data)) {
        output$graph_area_bench_histo <-
          plotly::renderPlotly(
            plot_bench_histo(
              benchmark_data,
              input$select_bench_histo,
    ...
    observeEvent({
      plotly_click_wo_warnings(sc = "bench_scatter")
    }, {
      bm <- isolate(benchmark_data())
      bm <- bm[["PCal"]]
      event.data <- plotly_click_wo_warnings(sc = "bench_scatter")
      showModal(modalDialog(plotly::renderPlotly({
    ...
    comparison <- observeEvent(input$start_compare, {
      tryCatch({
        ...
        if (input$use_generated_benchmark == TRUE) {
          b_o_tables <-
            import_benchmark(benchmark_data()$PCal,
                             options_path,
                             from_csv = FALSE,
          ...
          b_o_tables <-
            import_benchmark(benchmark_file(),
                             options_path,
                             from_csv = TRUE,
                             input$algorithm_input)
          b_table = b_o_tables$b_table
        ...
        #####################
        comparison_ug_g <-
          compare_peaks(b_table, ug_table, g_table, input$algorithm_input)
        ...
        shinyWidgets::sendSweetAlert(
          ...
          text = paste0(
            'Assessment has been finished in ',
            round(proc.time, 0),
            ' seconds. An overview is provided in panel "View NPP assessment"!'
          ),
          type = 'success',
          closeOnClickOutside = FALSE,
          showCloseButton = TRUE
    ...
    observeEvent(comparison_data(), {
      ...
      if (!is.null(comparison_data)) {
        comp.dt <-
          rbindlist(list(comparison_data$c_table, comparison_data$nf_b_table),
                    fill = TRUE)
        updateSelectInput(
          session,
          'mol_c',
          choices = as.character(unique(comp.dt$molecule_b)),
          selected = as.character(unique(comp.dt$molecule_b)[1])
        )
      }
    })
    ...
    observeEvent({
      ...
      if (!is.null(comparison_data)) {
        ...
        updateSelectInput(session,
                          'ia_c',
                          choices = sort(round_woe(unique(
                            comp.dt[molecule_b == input$mol_c &
                                      adduct_b == input$add_c]$isoab_b
                          ), 2), decreasing = TRUE))
      }
    })
    observeEvent({
      ...
      if (!is.null(comparison_data)) {
        output$graph_area_4 <-
          plotly::renderPlotly(
            plot_comp_peak_overview(comparison_data, input$mol_c, input$add_c, input$ia_c)
          )
      }
    })
    ...
    suppressWarnings(observeEvent(plotly_click_wo_warnings(sc = "scatter"), {
      #      delay(expr =({
      #Sys.time((0.1))
      ...
      CE_plot <-
        rbindlist(
          list(
            comparison_data$c_table[, Split_peak := FALSE],
            comparison_data$split_table[present_in_found == FALSE][, Split_peak := TRUE],
            comparison_data$nf_b_table[, Split_peak := FALSE]
          ),
          fill = TRUE
        )
      CE_plot <-
        CE_plot[, NPP_status := ifelse(
          !is.na(peak_area_ug),
          ifelse(Split_peak == "TRUE", 'Split', 'Found'),
          'Not Found'
        )]
      CE_plot <-
        unique(CE_plot,
      ...
      event.data <-
        suppressWarnings(plotly::event_data("plotly_click", source = "scatter", priority = "event"))
    ...
    observeEvent({
      comparison_data()
      input$PP_al_switch_hm
      input$PP_al_switch_hm_off
    }, {
      if (!is.null(comparison_data())) {
          ...
          output$graph_area_1 <-
            plotly::renderPlotly(
              plot_comp_missing_value_hm(
                comparison_data(),
                post_alignment = input$PP_al_switch_hm
              )
            )
        } else {
          output$graph_area_1 <-
    ...
    observeEvent({
      comparison_data()
      input$graph_select_input
      input$PP_al_switch_dist
    }, {
      if (!is.null(comparison_data())) {
        output$graph_area_3 <-
          plotly::renderPlotly(
            plot_comp_dist_of_found_peaks(
              comparison_data(),
              input$graph_select_input,
              choice_vector_comp = choice_vector_comp,
              post_alignment = input$PP_al_switch_dist
            )
          )
      }
    })
    ...
    observeEvent({
      ...
      if (!is.null(comparison_data())) {
        output$graph_area_2 <-
          plotly::renderPlotly(
            plot_comp_iso_pred_error(comparison_data(), post_alignment = FALSE) %>%
              plotly::event_register('plotly_click')
          )
      }
    })
    ...
    observeEvent(plotly_click_wo_warnings(sc = "IRbias"), {
      ...
      CE_plot <-
        rbindlist(list(comparison_data$c_table, comparison_data$nf_b_table),
                  fill = TRUE)
      event.data <-
        suppressWarnings(plotly::event_data("plotly_click", source = "IRbias", priority = "event"))
      #print(event.data)
      showModal(modalDialog(plotly::renderPlotly({
    ...
    observeEvent(comparison_data(), {
      ...
      if (!is.null(comparison_data)) {
        ...
        output$PP_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Peak picking step"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "Found peaks: ",
                                      ...
                                      " (",
                                      round_woe(result_list[["Before_alignment"]][["Found_peaks"]][["CI"]][4], 1),
                                      " - ",
                                      round_woe(result_list[["Before_alignment"]][["Found_peaks"]][["CI"]][5], 1),
                                      "%)",
                                      shiny::br(),
                                      "Missing peaks (high): ",
                                      ...
                                      result_list[["Benchmark"]][["BM_peaks"]],
                                      "(",
                                      round_woe(result_list[["Before_alignment"]][["Split_peaks"]][["CI"]][4], 1),
                                      " - ",
                                      round_woe(result_list[["Before_alignment"]][["Split_peaks"]][["CI"]][5], 1),
                                      "%)",
                                      shiny::br(),
                                      "Degenerated IR: ",
        ...
        output$A_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Alignment step"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "Min. errors: ",
                                      result_list[["Alignmnet"]][["Min.Errors"]][["count"]],
                                      " (",
                                      ...
                                      shiny::br(),
                                      "BM divergences: ",
                                      result_list[["Alignmnet"]][["BM_divergences"]][["count"]],
                                      " (",
                                      ...
                                      shiny::br(),
                                      "Lost peaks: ",
                                      result_list[["Alignmnet"]][["Lost_b.A"]][["count"]],
                                      " (",
        ...
        output$F_info <- shinydashboard::renderInfoBox({
          shinydashboard::infoBox(
            shiny::tags$p(style = "font-weight: bold; font-size: 110%", "Post alignment"),
            value = shiny::tags$p(style = "font-weight: normal; font-size: 100%;",
                                  shiny::HTML(
                                    paste(
                                      "Found peaks: ",
                                      ...
                                      result_list[["Benchmark"]][["BM_peaks"]],
                                      " (",
                                      round_woe(result_list[["After_alignmnet"]][["Found_peaks"]][["CI"]][4], 1),
                                      " - ",
                                      round_woe(result_list[["After_alignmnet"]][["Found_peaks"]][["CI"]][5], 1),
                                      "%)",
                                      shiny::br(),
                                      "Missing peaks (high): ",
    ...
    observeEvent(comparison_data(), {
      ...
      if (!is.null(comparison_data())) {
        output$error_count <-
          renderTable(comparison_data()$ali_error_table[Min.errors > 0 |
                                                          Lost_b.A > 0 | BM.div > 0])
      }
      #delay(expr =({
      #  options(warn = storeWarn)
    ...
    observeEvent(comparison_data(), {
      if (!is.null(comparison_data())) {
        error_molecules <-
          unique(as.character(comparison_data()$ali_error_table[Min.errors > 0 |
                                                                  Lost_b.A > 0 | BM.div > 0, Molecule]))
        no_error_molecules <-
          unique(as.character(comparison_data()$ali_error_table[Min.errors == 0 &
                                                                  Lost_b.A == 0 | BM.div == 0, Molecule]))
        choices <-
          list(
            'Errors:' = as.list(error_molecules),
            'No errors:' = as.list(no_error_molecules)
          )
    ...
    observeEvent({
      ...
      if (!is.null(comparison_data())) {
        error_adducts <-
          as.character(comparison_data()$ali_error_table[(Molecule == input$mol_a) &
                                                           (Min.errors > 0 | Lost_b.A > 0 | BM.div > 0), Adduct])
        no_error_adducts <-
          as.character(comparison_data()$ali_error_table[(Molecule == input$mol_a) &
                                                           (Min.errors == 0 & Lost_b.A == 0 | BM.div == 0), Adduct])
        choices <-
          list(
            'Errors:' = as.list(error_adducts),
            'No errors:' = as.list(no_error_adducts)
          )
        shinyWidgets::updatePickerInput(session = session,
                                        inputId = 'add_a',
                                        choices = choices)
    ...
    observeEvent(comparison_data(), {
      if (!is.null(comparison_data())) {
        output$graph_hm_split <-
          plotly::renderPlotly(Alignment_error_plot(
            comparison_data(),
            mol = input$mol_a,
            add = input$add_a
          ))
