...
3. (optional) Download the already prepared benchmark "Benchmark.csv" if you do not want to do benchmark generation part of the example from [ucloud](https://ucloud.univie.ac.at/index.php/s/JFcVukwxKBLSRsC)
...
1. It makes always sense to first check the final output in the <i>Post Alignment</i> box (If the final output looks good we generally do not need to waste time on understanding if the intermediate steps worked well). There we can see that about 71-88% of peaks have been detected, which is not really perfect (especially if we consider that we also used a fill gaps algorithm). The isotopologue ratio (IR)-metric reports 19-40% degenerated IR, which we would consider to be quite a lot. Hence we need to look at the other steps.
