...
3. (optional) Download the already prepared benchmark "Benchmark.csv" if you do not want to do the benchmark generation part of the example from [ucloud](https://ucloud.univie.ac.at/index.php/s/JFcVukwxKBLSRsC)
4. Download the XCMS-output files "XCMS_aligned_run.csv" and "XCMS_unaligned_run.csv" files from [ucloud](https://ucloud.univie.ac.at/index.php/s/BjF24f7q4jeFJH3). Alternatively, you can also process the mzML files downloaded before by yourself. If you do so make sure to follow the instructions given in the readme section "Exporting NPP outputs from different tools".
...
1. If you processed all 30 mzML files you should have generated a benchmark containing 48 different molecules with 145 different features (including all adducts and isotopologues) resulting in 2736 peaks in total.
...
Afterward, assessment can be started by pressing the blue button on the bottom of the page.
...
1. It makes always sense to first check the final output in the <i>Post Alignment</i> box (If the final output looks good we generally do not need to waste time on understanding if the intermediate steps worked well). There we can see that about 71-88% of peaks have been detected, which is not perfect (especially if we consider that we also used a fill gaps algorithm). The isotopologue ratio (IR)-metric reports 19-40% degenerated IR, which we would consider being quite a lot. Hence we need to look at the other steps.

2. The <i>Peak Picking</i> box shows that 2323/2736 peaks have been found, which should correspond to about 82-89% of all peaks in the raw data. This is in the same range as the final output (we would consider that to be not so problematic since the fill gaps algorithm should be able to improve that at least in part). The IR-metric looks quite good with only up to 4% of IR being degenerated. However, up to 11% of matches led to split peaks which is quite a lot. This is problematic since they tend to cause problems for downstream alignment processes. More specifically they are often in competition with more "properly" picked peaks during the alignment process and also "sabotage" the fill gaps algorithm.
...
<b>We consider the high number of degenerated IR in the aligned output as not acceptable. As mentioned the peak picking step might have been problematic. We will therefore try to improve it.</b>
...
4. Select benchmark file: the exported benchmark as csv (if you did not process all 30 files or skipped the benchmark generation step please use the benchmark provided in [ucloud](https://ucloud.univie.ac.at/index.php/s/JFcVukwxKBLSRsC))
...
2. Also in the <i>Peak Picking</i> box both of those metrics did not change much. However, we have a lot fewer split peaks now (1-3%) which is going in the right direction.
...
1. In the <i>Post Alignment</i> box we see that now about 91-98% of peaks have been detected. Which is quite some improvement. Also, the proportion of degenerated IR decreased to 6-14%. The Missing peaks classification shows that no classification was possible which is most likely because whole features have gone missing rather than random peaks. This can be confirmed by scrolling down to the <i>Nature of missing values</i> section, setting the slider to <i>after Alignment</i> and turning the plotting function on.

2. We did not change anything about the peak picking step so everything stayed the same there.

3. The alignment improved quite a bit to about 4-9% lost peaks.

<b>Although this was much better than Example 1 there is still room for improvement. If you want you can try to do better using XCMS or another tool (all tools listed in the readme on [GitHub](https://github.com/YasinEl/mzRAPP) are supported by mzRAPP).</b>
