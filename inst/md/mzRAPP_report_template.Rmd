...
result_list <- generate_results_text(params$comp_d)
```

Assessment of non targeted data pre-processing (NPP) was performed via mzRAPP (version 1.0). mzRAPP is based on user provided manually curated information on a number of chromatographic peaks with known molecular composition which is then automatically checked and extended in order to generate a benchmark data set (BM). This BM is then applied as reference to the NPP output in order to extract a number of performance metrics for the overall NPP output as well as individual steps. Additional information on the underlying processes are provided [here](https://github.com/YasinEl/mzRAPP).
<br>
<br>

# NPP performance metrics
Different NPP performance metrics are assessed automatically via mzRAPP. During the evaluation process mzRAPP is considering a multitude of orthogonal information for benchmark generation as well as for determination of NPP performance metrics. This is done in order to avoid relying completely on user provided or mzRAPP-generated information. 
...
dt_peak_presence <- data.table(metrics = c("Found_peaks_pp", "Found_peaks_al"),
                               values = c(paste0(result_list[["Before_alignment"]][["Found_peaks"]][["count"]],
                                          ...
                                          round(result_list[["Before_alignment"]][["Found_peaks"]][["CI"]][5],1),
                                            "%)"),
                                          paste0(result_list[["After_alignmnet"]][["Found_peaks"]][["count"]],
                                            "/",
                                            result_list[["Benchmark"]][["BM_peaks"]],
                                            " (",
                                            round(result_list[["After_alignmnet"]][["Found_peaks"]][["CI"]][4],1), " - ", 
                                          round(result_list[["After_alignmnet"]][["Found_peaks"]][["CI"]][5],1),
                                            "%)"))
                               )
...
dt_peakQual <- data.table(metrics = c("IR_bias>20%_pp", "IR_bias>20%_al"),
                          values = c(paste0(result_list[["Before_alignment"]][["IR_quality"]][["Error_inc_above20pp"]][["count"]],
                                     ...
                                     round(result_list[["Before_alignment"]][["IR_quality"]][["Error_inc_above20pp"]][["CI"]][5], 
                                           1), "%)"),
                                     paste0(result_list[["After_alignmnet"]][["IR_quality"]][["Error_inc_above20pp"]][["count"]],
                                            "/",
                                            result_list[["After_alignmnet"]][["IR_quality"]][["Error_inc_above20pp"]][["count"]] + 
                                       result_list[["After_alignmnet"]][["IR_quality"]][["Error_inc_below20pp"]],
                                            " (",
                                     round(result_list[["After_alignmnet"]][["IR_quality"]][["Error_inc_above20pp"]][["CI"]][4], 
                                           1), " - ",
                                     round(result_list[["After_alignmnet"]][["IR_quality"]][["Error_inc_above20pp"]][["CI"]][5], 
                                           1), "%)")
                                     )
                          )
...
# Generated benchmark
Key metrics of the mzRAPP generated benchmark are summarized in the following. 
<div class = "row">
<div class = "col-md-4">
```{r message=FALSE, warning=FALSE, echo=FALSE}
bm <- rbindlist(list(params$comp_d$c_table, params$comp_d$nf_b_table), fill = TRUE, use.names = TRUE)

dt_benchmark <- data.table(metrics = c("molecule count", "sample count", "peak count", "med_scan_acqu._rate [1/s]"),
                           values = c(round(length(unique(bm[Split_peak == FALSE & main_peak == TRUE]$molecule_b)), 0),
                                      round(length(unique(bm[Split_peak == FALSE & main_peak == TRUE]$sample_name_b)),0),
                                      round(nrow(bm[Split_peak == FALSE & main_peak == TRUE]),0),
...
<div class = "col-md-8">
```{r message=FALSE, warning=FALSE, echo=FALSE}
subplot(plot_bench_histo(benchmark_data = params$comp_d, 
                         var = "peak_height_b", 
